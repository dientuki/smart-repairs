...
export const OrderCard = ({
  ...
  return (
    ...
    >
      <div
        className='relative w-full aspect-video bg-cover bg-no-repeat'
        style={{ backgroundImage: `url(${order.deviceTypeImage})` }}
      >
        <img
          src={order.brandImage}
          alt={order.brand}
          className='absolute top-2 left-2 h-8 w-auto object-contain'
        />
      ...
      <div className='flex flex-col gap-1 p-2 text-base text-gray-950 dark:text-white'>
        ...
        {order.deviceTechName && (
          <div>
            {order.deviceType}: {order.deviceTechName}
          </div>
        )}
