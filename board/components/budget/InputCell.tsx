...
export const InputCell = ({ getValue, row, column, table }: InputCellProps) => {
  ...
  const isHidden =
    (row.index === 0 || row.index === 1) && column.id === "quantity";
    ...
    <Controller
      ...
      render={({ field }) => (
        <Input
          ...
          className={`${column.id == "quantity" ? "text-center" : ""} rounded-lg bg-gray-50 border text-gray-900 focus:ring-blue-500 focus:border-blue-500 block w-full text-base border-gray-300 p-2.5 ${
            errorMessage
