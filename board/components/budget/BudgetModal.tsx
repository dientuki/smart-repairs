...
export const BudgetModal = () => {
  ...
  useEffect(() => {
    initialValues(modal.params.order).finally(() => {
      setIsLoading(false);
      setData([...defaultData]);
    });
  }, []);
  ...
  const table = useReactTable({
    ...
    meta: {
      ...
      addRow: () => {
        const canAdd = data.every(
          (item) => item.itemId === "init" || item.serviceId !== "",
        );
        if (canAdd) {
  ...
  return (
    <ModalLayout
      ...
      width='70vw'
      title={
        <h2 className='text-2xl font-medium leading-6 text-gray-900 mb-4'>
          Presupuestar equipo
        </h2>
      }
    >
      {!isLoading && (
        ...
        >
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <tr key={headerGroup.id}>
                  {headerGroup.headers.map((header) => (
                    <th
                      key={header.id}
                      className={`${header.column.columnDef.meta?.className || ""} py-1 px-2`}
                    >
            ...
            <tbody>
              ...
              <tr>
                <td
                  ...
                  align='right'
                  className='border-t-4 border-green-500 py-3'
                >
                  <div
                    className='first-letter:uppercase cursor-pointer float-right'
          ...
          <div className='mt-4'>
            <ActionButton customClass=' mt-6' type='submit'>
              Done
            </ActionButton>
          </div>
        </form>
